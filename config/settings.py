...
# .strip() НЕ косметика: хвостовой перевод строки или пробел в переменной
# окружения (Coolify так и хранит) уходил в заголовок `Bearer <токен>\n`, и
# Tinkoff отвергал КАЖДЫЙ запрос — стрим брал токен отсюда, минуя TinkoffClient.
...
# ─── Постоянное соединение с биржей (src/collector/stream.py) ─────────────────
#
# Опрос REST по кругу давал ядру снимок раз в ~5 минут, а хвосту раз в ~43
# (замер 01.08). В хвосте были MVID, DATA, SGZH, UGLD — те, что ходят сильнее
# всего. Стрим держит все бумаги одновременно: лимит 300 подписок на соединение,
# нам нужно 160 = 80 бумаг на два типа данных.
#
# Флаг ВЫКЛЮЧЕН по умолчанию сознательно. Токена в песочнице нет, сетевую часть
# нельзя проверить до деплоя, а на этих данных торгуют реальными деньгами.
# Опрос продолжает работать рядом и остаётся страховкой.
STREAM_ENABLED = os.getenv("STREAM_ENABLED", "false").lower() == "true"
STREAM_DEPTH = int(os.getenv("STREAM_DEPTH", "20"))        # 1|10|20|30|40|50
STREAM_FLUSH_SEC = int(os.getenv("STREAM_FLUSH_SEC", "20"))
BOOK_MINUTE_KEEP_DAYS = int(os.getenv("BOOK_MINUTE_KEEP_DAYS", "90"))
