...
class ClaudeAgent:
    ...
    async def batch_scan(self, market_context: str, briefs: list,
        ...
        def _news_s(n, lag):
            ...
            return f" nws{n}" if lag is None else f" nws{n}/{lag:+.0f}м"


        def _sb_s(v):

            """Основа настроения для строки batch: пусто, если неизвестна."""

            return "" if not v else f" sb:{v}"
        ...
        legend = ("Формат строки: ТИКЕР p<цена> <позиция к VWAP> <режим> adx rsi atr "
                  ...
                  "rr<R:R ДНЕВНОГО плана техники> srr<R:R ИНТРАДЕЙ-сетапа: именно того входа, о котором говорит поле сетапа; если пусто — сетап не выдан> <статус входа> ob:<давление стакана>/<отношение ОБЪЁМОВ bid к ask — НЕ спред, 1.7 значит перевес покупателей 1.7:1>/<ликвидность> sp<спред, проценты: по ликвидным бумагам 0.004-0.10%> "
                  ...
                  "sb=основа настроения: text=чаты/новости, orderbook=только "
                  "давление стакана (тот же сигнал, что в блоке стакана — не "
                  "считать его вторым независимым подтверждением), mixed=смешано "
