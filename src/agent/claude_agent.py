...
ШАГ 6 — РЕШЕНИЕ (инвалидация-first): сначала определи, где сделка НЕВЕРНА (уровень слома) → стоп за него → цель на ближайшем логичном уровне → R:R. Вход ТОЛЬКО если R:R ≥ 1.5 И конфлюенс ≥ 3 И риск-гейт пройден; иначе — наблюдать. Риск на сделку 1% (размер считается от стопа). Вход задавай лимиткой на уровне, не «по рынку».
...
{{
  ...
  "stop": "уровень стопа/инвалидации числом или null",
  "target": "уровень цели числом или null",
  "rr": "R:R числом или null",
  "size": "full|half|none",
...
Помни: дефолт — neutral/none. Вход только если R:R ≥ 1.5 И конфлюенс ≥ 3 И риск-гейт passed."""

        try:
            result = await self._ask(system, user, max_tokens=1300)
            import json
            start = result.find("{")
            end   = result.rfind("}") + 1
            if start >= 0 and end > start:
