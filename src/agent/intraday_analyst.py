...
def compute_intraday_context(candles: dict, minute_of_day: int,
    ...
    # Структура ПОСЛЕДНИХ ТРЁХ ЧАСОВ, а не дня. Здесь стояло «структура дня», и
    # это была неправда: 36 пятиминуток — это три часа. Замер на 27 546 срезах
    # (12 бумаг × 181 день, каждый час сессии): совпадает с настоящей структурой
    # сессии в 54.4% случаев. Утром почти всегда (окно ≈ вся сессия), к вечеру
    # падает до 42%. В 3.5% случаев показывает ПРОТИВОПОЛОЖНОЕ направление.
    #
    # Окно оставлено трёхчасовым намеренно. Проверял, что даст структура всей
    # сессии: доходность следующего часа, разница «вверх минус вниз» — окно 3ч
    # даёт +0.0004% (t=0.04), вся сессия +0.0153% (t=1.27). Оба результата шум,
    # оба втрое меньше издержек 0.05%. Менять окно не на что: предсказательной
    # силы нет ни у одной версии. Поэтому чинится ПОДПИСЬ, а не число — чтобы
    # потребитель не считал трёхчасовую структуру характеристикой дня.
    _struct = None
    _struct_bars = 0
    if len(c) >= 36:
        _struct_bars = 36
        ...
        if _lows[2] >= _lows[1] >= _lows[0] and _highs[2] >= _highs[1]:
            ...
            _struct = "боковик"
    # Какую долю уже прошедшей сессии реально покрывает окно. Утром близко к 1.0
    # (структура и правда про сессию), вечером ~0.2 — и тогда называть её
    # структурой дня нельзя. Число отдаём наружу, чтобы это было видно.
    _struct_cover = round(min(1.0, _struct_bars / len(c)), 2) if _struct_bars and c else None
    ...
    return {
        ...
        "market_state": None,
        # Имя поля говорит, ЧТО это: структура последних трёх часов. Прежнее
        # day_structure оставлено рядом как псевдоним, чтобы не сломать
        # потребителей, но новый код должен читать structure_3h.
        "structure_3h": _struct,
        "structure_3h_bars": _struct_bars or None,
        # Доля прошедшей сессии, покрытая окном: 1.0 утром, ~0.2 вечером.
        "structure_session_cover": _struct_cover,
...
async def intraday_outcome(ticker: str, start_iso: str, direction: str,
    ...
    # ATR для трейла — ПО БАРАМ ДО ВХОДА, а не по всей выборке.
    #
    # Здесь стояло intraday_atr(highs, lows, closes) по всему окну. Функция берёт
    # последние 14 баров, а окно тянется до МОМЕНТА ОЦЕНКИ — то есть трейл сделки,
    # открытой вчера, считался по сегодняшней волатильности. Заглядывание вперёд
    # в чистом виде: в момент сделки этого числа не существовало.
    #
    # Направление искажения не одностороннее (сегодня тише — трейл жмётся и режет
    # прибыль; сегодня резче — трейл распускается и держит дольше), но результат
    # один: R в журнале НЕВОСПРОИЗВОДИМ в реальном времени. Учиться на таких
    # числах нельзя — правило, выведенное из них, в живой торговле не повторится.
    _entry_i = path[0] if path else None
    if _entry_i:
        atr = iv.intraday_atr(highs[:_entry_i], lows[:_entry_i], closes[:_entry_i]) or R_unit
    else:
        atr = R_unit
