...
async def screen_ticker(ticker: str, aggregator) -> Optional[dict]:
    ...
    res["brief"] = {
        ...
        # Темп объёма незавершённого дня. Головная цифра rvol утром считается по
        # ПОСЛЕДНЕМУ ЗАВЕРШЁННОМУ дню, поэтому темп нужен отдельно — иначе модель
        # не увидит, что сегодня объём идёт вдвое быстрее обычного.
        "pace": (technical or {}).get("pace_rel"),
