...
def completeness(rows, day, at=None):
    ...
    могут на короткое время писать в одну минуту.
    """
    ...
    for r in rows or []:
        ts = r.get("ts")
        if not ts or not ts.startswith(day) or ts in seen:
            continue
        seen.add(ts)
...
def block(rows, day, at=None):
    """Общая справка по одной таблице: сколько минут, какая последняя,
    какой у неё возраст и насколько ряд полон."""
