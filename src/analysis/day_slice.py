...
src.analysis.sessions это правило соблюдает — он читает только окружение.
"""
from datetime import datetime, timedelta

from src.analysis import sessions
...
#
#  РАНЬШЕ ОНИ БЫЛИ ЗАШИТЫ ЗДЕСЬ и расходились с config/settings.py: утро
#  начиналось в 06:50 (аукцион считался сессией), основная кончалась в 18:39
#  вместо 18:49, вечер — в 23:49 вместо 23:49 (совпало случайно). Из-за
#  расхождения полнота основной сессии считалась от 520 минут вместо 530,
#  то есть одиннадцать минут дня выпадали из учёта молча.
#
#  Теперь границы приходят из одного места на весь проект. Аукцион
#  (06:50-07:00 и 09:50-10:00) и перерыв (18:50-19:00) в торговые окна НЕ
#  входят: непрерывных торгов там нет, и требовать в них полноту ряда
#  значит записывать биржевой аукцион себе в дыры.
SESSIONS = sessions.windows_inclusive_ru()
