...
def no_data_note(running: bool, phase: str, fresh_60s: int = 0) -> str:
    """
    Почему у сканера нет баров. Три РАЗНЫЕ причины, а не одна фраза.

    Прежняя подпись «минутной истории ещё нет — стрим только поднялся» была
    одинакова во всех трёх случаях, и один из них — неисправность:

        поток не поднялся                    сломано
        поток жив, рынок закрыт              норма
        поток жив, рынок ОТКРЫТ, пакетов 0   сломано, и это самый дорогой случай

    В 11:00 буднего дня третий спрятался бы за той же фразой. Это та же
    неразличимость, из-за которой «0 событий» на пустой памяти после деплоя было
    принято за спокойный рынок: молчание обязано называть свою причину.
    """
    if not running:
        return "поток не поднялся — это неисправность, смотрите /api/stream/health"
    if phase in ("closed", "pre", "break"):
        word = {"closed": "рынок закрыт", "pre": "аукцион открытия",
                "break": "перерыв между сессиями"}[phase]
        return f"{word} — история дня ещё не набрана"
    if not fresh_60s:
        return ("рынок открыт, но за минуту не пришло НИ ОДНОГО пакета — "
                "это неисправность, а не тишина")
    return "история дня ещё набирается — нужно несколько закрытых баров"
