...
class LevelTracker:
    ...
    def on_book(self, ticker: str, minute: str, bids: list, asks: list,
        ...
        for side, book in (("bid", bids), ("ask", asks)):
            ...
            # Сначала БЛИЗОСТЬ, потом размер — и порядок здесь важен.
            #
            # Первая версия считала «крупнейшего» по всем уровням стороны, включая
            # далёкие. Тогда плита в двух процентах от цены поднимала планку для
            # уровней у самой цены и ГЛУШИЛА их журналы: как раз те, куда цена
            # придёт, оставались без истории. Нашлось собственным тестом.
            # Ближайшие NEAR_LEVELS цен к лучшей на своей стороне. Не процент:
            # процент у SBER по 280 ₽ это пять тиков, а у FEES по 0.05 ₽ — меньше
            # тика, и журнал молчал бы совсем.
            by_near = sorted(present.items(),
                             key=lambda kv: -kv[0] if side == "bid" else kv[0])
            close = dict(by_near[:lh.NEAR_LEVELS])
            ranked = sorted(close.items(), key=lambda kv: -kv[1])
            biggest = ranked[0][1] if ranked else 0
            floor_qty = biggest * lh.ENTER_SHARE
            # БЛИЗОСТЬ К ЦЕНЕ — первый отсев, и он важнее размера.
            #
            # Смысл истории уровня в том, чтобы увидеть, что было ДО теста.
            # Уровень в двух процентах от цены за минуту не протестируют, и его
            # журнал занимает слот впустую. 03.08 на живой бирже без этого
            # отсева журналы вытеснялись 1 280 раз в минуту и у 15 уровней из 16
            # история была ПУСТА — дополнение существовало только на бумаге.
            # Отсчёт от лучшей цены СВОЕЙ стороны, а не от середины рынка: у
            # бумаги с широким спредом середина далека от обеих сторон, и обе
            # вылетали из окна разом. Цена приходит к уровню со своей стороны.
            #
            # bb и ba взяты СВЕРХУ и уже округлены. Своя копия без округления
            # вернула бы баг, против которого стоит отдельный тест: цены уровней
            # лежат в ключах как round(..., 6), и «цена дошла до уровня» не
    ...
    def state(lv: dict) -> str:
        ...
        а не прогноз:
            ...
            failed      тест был и закончился проходом
            defended    тест был и ЗАКОНЧИЛСЯ отступлением цены
            testing     цена у уровня ПРЯМО СЕЙЧАС, исход неизвестен
        ...
        «Выдержал» требует ЗАВЕРШЁННОГО теста. Найдено на живых данных 02.08: у
        LKOH bid 4587 стояло «выдержал» при счёте тестов 1 и нулях в обеих
        колонках исхода — то есть цена стояла у уровня прямо в тот момент, и
        ничего он ещё не выдержал. Незавершённый тест, выданный за пройденный, —
        то же переобещание, что и «сильный», только незаметнее.
        ...
        if lv.get("broken"):
            return "broken"
        if lv.get("test_failed", 0):
            return "failed"
        if lv.get("test_held", 0):
            return "defended"
        if lv.get("in_test"):
            # Тест идёт прямо сейчас: цена у уровня, исход неизвестен.
            return "testing"
