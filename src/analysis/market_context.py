...
def changes(minutes: dict, back: int = 1) -> dict:
    ...
    `minutes` — тикер -> последовательность (метка минуты, закрытие), от старых
    к новым. Берётся то, что реально есть: если у бумаги истории меньше, чем
    просят, она в ответ не попадает. Подставлять ноль нельзя — «не изменилась» и
    «не знаем» это разные вещи.
    """
    out = {}
