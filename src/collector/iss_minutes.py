...
1. `turnover_error` — сверка с value самого ISS. Она ловит битые цены и
   рассогласованный оборот, но НЕ ЛОВИТ НЕВЕРНУЮ ЛОТНОСТЬ:

       volume = штуки / лот,   _rub = volume × лот × цена = штуки × цена

   Лот сокращается. Сумма сойдётся хоть при лоте 1, хоть при 1000.
   Это было сказано неверно в первой версии этого файла.

2. `compare_to_db` — сверка с тем, что СТРИМ записал за тот же день.
   Вот она единицы и ловит: разойдёмся в лотности — медиана отношения
   будет ровно лотом или его обратным, а не единицей.
...
#  Меньше этого числа общих минут — сравнивать нечего: на десятке точек
#  медиана скажет больше про случай, чем про единицы.
MIN_COMMON = 20
...
def turnover_error(bars: list, lot: int = 1) -> Optional[float]:
    ...
    ЧТО ЭТО ЛОВИТ: битые цены, смещённые столбцы, перепутанные value/volume.

    ЧЕГО ЭТО НЕ ЛОВИТ: неверную лотность. Если делить и умножать на
    ОДНО И ТО ЖЕ число, оно сокращается. Для лотности есть compare_to_db.
...
def compare_to_db(iss_bars: list, db_rows: list) -> dict:
    ...
    Это единственная проверка, которая действительно ловит лотность:
    два независимых источника о ОДНОЙ и той же минуте. Отношение
    берётся поминутно, итог — МЕДИАНА, а не среднее: один аукционный
    выброс или одна пропущенная стримом минута не должны решать вердикт.

    Несколько процентов расхождения — норма: стрим мог подняться среди
    дня и пропустить сделки. Разы — не норма никогда.
    """
    mine = {}
