...
ТРИ ЛОВУШКИ, И ВСЕ ТРИ МОЛЧАЛИВЫЕ.
...
3. СТРАНИЦА В 500 СТРОК. ISS отдаёт не день, а страницу. Торговый день
   Мосбиржи с утренней и вечерней сессиями длиннее 500 минут (в базе
   стрима бывает 777). Запрос без перебора start= приносит ровно 500
   минут — и это выглядит как полноценный день: порог MIN_BARS_DAY пройден,
   сверка оборота сойдётся (она считается по тем же самым барам).
   А отсутствующие минуты всегда на ОДНОМ краю сессии, то есть профиль
   получил бы ровно тот перекос по времени суток, от которого лечимся.
...
1. `turnover_error` — сверка с value самого ISS. Ловит битые цены и смещённые
   ...
   этого файла было сказано обратное, и это было неверно. Она же не ловит
   и недобранные страницы: обе стороны сверки берутся из того же ответа.
...
#  Сколько строк ISS отдаёт за один запрос. Не настройка, а наблюдение:
#  ровно 500 минут пришло в шести запросах подряд на трёх бумагах.
PAGE = 500

#  Предел перебора страниц за один день. В сутках 1440 минут, так что
#  четырёх страниц хватает с запасом. Это защита от вечного цикла,
#  если ISS вдруг станет отдавать одну и ту же страницу.
MAX_PAGES = 4
...
def candles_url(ticker: str, day: str, board: str = "TQBR",
                start: int = 0) -> str:
    """Адрес минутных свечей за один день. Без токена и без подписи.

    `start` — номер первой строки страницы. День Мосбиржи длиннее
    одной страницы, так что без перебора здесь теряется край сессии.
    """
    return (f"{ISS.format(board=board, ticker=ticker)}"
            f"?iss.meta=off&interval=1&from={day}&till={day}"
            f"&start={int(start)}")
...
def page_is_full(payload: dict) -> bool:
    """Страница набита докрая — значит, есть следующая.

    Считаются СЫРЫЕ строки ответа, а не разобранные бары: если пару
    строк отбрасывает bar_of, страница от этого не перестаёт быть полной,
    а перебор оборвался бы на середине дня.
    """
    return len(rows_of(payload)) >= PAGE
...
def bars_of(payload: dict, lot: int = 1) -> list:
    """
    Один ответ — в список баров, по времени, без повторов.
    ...
    """
    return bars_of_pages([payload], lot=lot)


def bars_of_pages(payloads: list, lot: int = 1) -> list:
    """
    НЕСКОЛЬКО страниц одного дня — в один список баров.

    Склейка идёт по ключу минуты, поэтому перехлёст страниц безопасен:
    повторная минута замещает саму себя, а не удваивается.
    """
    by_ts = {}
...
def turnover_error(bars: list, lot: int = 1) -> Optional[float]:
    ...
    ЧЕГО НЕ ЛОВИТ: неверную лотность (она сокращается) и недобранные
    страницы (обе стороны сверки — из одного и того же ответа).
