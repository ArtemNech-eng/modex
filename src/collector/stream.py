...
# ПОЧЕМУ поток не поднялся. Без этого /api/stream/health отвечал «включён, но ещё
# не поднялся или упал при старте» — фраза, под которую подходит и падение при
# старте, и молчаливый выход конвейера, и транзиентный отказ Tinkoff. Они требуют
# разного, и различать их надо снаружи, а не в логе контейнера.
#
# 03.08 в 15:57 прод стоял без данных 15 минут, а health говорил ровно эту фразу.
