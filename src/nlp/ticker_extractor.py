...
# Найдено 30.07 в живой ленте:
#   «Мишустин выделил 2,6 млрд на достройку трёх самолетов Ил-114-300» -> SMLT
#   «Mizuho выделил топ-пик в биотехе»                                 -> PIKK
# Первое — прямое следствие моей же правки того же дня: терпимость к падежам
# превратила «самолет» в «самолетов». Второе — дефис режет слово, и граница слова
# находит «пик» внутри «топ-пик». Проверено также: «полёт на самолете»,
# «магнит на холодильник», «северный полюс», «пик нагрузки» — все давали тикер.
#
# Различитель: компания пишется с БОЛЬШОЙ буквы, обычное слово — с маленькой.
# Для этих псевдонимов ищем в ИСХОДНОМ тексте с заглавной, без окончаний и не
# внутри дефисного сложения.
AMBIGUOUS_ALIASES = {"пик", "самолет", "самолёт", "магнит", "полюс"}
...
def extract_tickers(text: str) -> list[str]:
    ...
    for alias, ticker in sorted(RUSSIAN_ALIASES.items(),
        ...
        if alias in AMBIGUOUS_ALIASES:
            ...
            pat = (r"(?<![-\w])(?:" + re.escape(cap) + r"|"
                   + re.escape(alias.upper()) + r")(?![-\w])")
            m = re.search(pat, text)
            if m:
                found.add(ticker)
                st, en = m.start(), m.end()
                consumed = consumed[:st] + " " * (en - st) + consumed[en:]
            continue
        stem = alias
        tail = ""
