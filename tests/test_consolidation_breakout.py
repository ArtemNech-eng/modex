...
def test_wired_into_context():
    """Сетап должен участвовать в выборе и записывать причину отказа."""
    import src.agent.intraday_analyst as ia
    n = 30
    dates = [f"2026-07-30T{13 + i // 12:02d}:{(i * 5) % 60:02d}:00+03:00" for i in range(n)]
    # утро низко (VWAP внизу), затем узкое сжатие и пробой на объёме
    highs = [37.5] * 20 + [38.29] * 8 + [38.37, 38.37]
    lows = [37.3] * 20 + [38.01] * 8 + [38.28, 38.28]
    closes = [37.4] * 20 + [38.15] * 8 + [38.35, 38.35]
    vols = [100] * 28 + [400, 400]
    c = {"open": closes[:], "high": highs, "low": lows, "close": closes,
         "volume": vols, "dates": dates}
    ctx = ia.compute_intraday_context(c, 14 * 60)
    assert ctx["setup"] == "consolidation_breakout" or ctx.get("breakout_blocked"), \
        (ctx["setup"], ctx.get("breakout_blocked"), ctx.get("note"))
