...
def test_long_never_in_validated_set():
    ...
    import src.agent.intraday_analyst as ia
    n = 29
    # Тело дня намеренно скромное: сжатие 0.15 руб должно составлять заметную
    # долю дневного хода. С прежним телом 37.30-38.36 доля выходила 14.2%, и
    # сетап отсекался фильтром «сжатие как доля дневного диапазона»
    # (tests/test_width_day_share.py) — тест падал не по своей теме.
    h = [37.95] * 20 + [38.20] * 8 + [38.36]
    l = [37.80] * 20 + [38.05] * 8 + [38.24]
    c = [37.88] * 20 + [38.12] * 8 + [38.34]
    v = [100] * 28 + [400]
    HH = 7
    dates = [f"2026-07-30T{HH + i // 12:02d}:{(i * 5) % 60:02d}:00+03:00" for i in range(n)]
    cc = {"open": c[:], "high": h, "low": l, "close": c, "volume": v, "dates": dates}
