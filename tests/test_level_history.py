...
def test_far_level_gets_no_log():
    ...
    Причина была не в потолке, а в критерии. «Сопоставим с крупнейшим на своей
    стороне» работало на синтетике, где размеры падали резко; на живой бирже
    распределение плоское, и в загруженную минуту события получают 94 уровня
    (p90) при двенадцати слотах.

    Замер расстояний до цены по десяти бумагам: внутри 0.2% — 5% уровней, внутри
    0.5% — 18%, внутри 1% — 40%. То есть журналы велись в основном по уровням,
    до которых цена не дойдёт, а смысл истории уровня — увидеть, что было ДО
    теста.
    """
    tr = tracker(history=lh.LevelLog(top_levels=20))
    # Стакан глубже окна: восемь бидов при NEAR_LEVELS = 6. Дальний КРУПНЕЕ всех
    # и всё равно журнала не получает — а заодно не поднимает планку размера для
    # ближних, потому что планка считается только среди них.
    bids = [(round(100.0 - 0.01 * i, 6), 1000) for i in range(8)]
    bids[-1] = (bids[-1][0], 90000)              # самый далёкий — самый крупный
    book(tr, T0, bids, asks=[(100.01, 1000)])
    assert lh.NEAR_LEVELS == 6, "тест написан под окно в шесть уровней"
    assert key(100.0) in tr.history.log, "лучший бид журнал получает"
    assert key(round(100.0 - 0.01 * 5, 6)) in tr.history.log, "шестой ещё внутри"
    assert key(bids[-1][0]) not in tr.history.log, \
        "восьмой журнала не получает, даже будучи крупнейшим"
