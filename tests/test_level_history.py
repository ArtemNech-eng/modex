...
def test_only_top_levels_start_a_log():
    ...
    Фикстура держит достаточно уровней, чтобы отбор был ВИДЕН. Прежняя ставила
    два бида, и при WALLS = 3 в «крупнейшие три» попадали оба — тест проверял не
    своё утверждение. Третий случай за день, когда фикстура мала для правила.
    """
    tr = tracker(history=lh.LevelLog(top_levels=1))
    bids = [(round(100.0 - 0.01 * i, 6), 5000 if i == 0 else 10)
            for i in range(14)]
    book(tr, T0, bids, asks=[(101.0, 10)])
    logged = {k[2] for k in tr.history.log if k[1] == "bid"}
    assert 100.0 in logged, "лучший и крупнейший"
    assert len(logged) < len(bids), "не все двадцать: журналов %d из %d" % (
        len(logged), len(bids))
    assert round(100.0 - 0.01 * 13, 6) not in logged, "последний журнала не заводит"
...
def test_eviction_takes_the_stalest_not_the_biggest():
    ...
    # Цены СОСЕДНИЕ, по тику. Прежняя фикстура ставила уровни на 100.0 и 99.0 —
    # это процент друг от друга, тогда как соседние уровни стакана отстоят на
    # тик, то есть на сотую процента. Отбор по близости к цене справедливо
    # выбросил бы такой «уровень», и тест падал не на своём предмете.
    book(tr, T0, [(100.0, 5000)])                          # заведён первым
    book(tr, T0 + 1, [(100.0, 5000), (99.99, 4000)])       # второй
    # Первый продолжает жить: у него свежее событие.
    tr.on_trade("SBER", 100.0, 2000)
    book(tr, T0 + 5, [(100.0, 3000), (99.99, 4000)])
    # Третий уровень вытеснит самый давний — это 99.99, а не активный 100.0.
    book(tr, T0 + 6, [(100.0, 3000), (99.99, 4000), (99.98, 9000)])
    assert key(100.0) in tr.history.log, "активный уровень остался"
    assert key(99.98) in tr.history.log
...
def test_only_relatively_big_levels_get_a_log():
    ...
    Фикстура держит ДОСТАТОЧНО соперников: при WALLS = 3 и трёх уровнях в стакане
    в «крупнейшие три» попадает всё, и тест проверял бы не своё утверждение.
    """
    tr = tracker(history=lh.LevelLog(top_levels=5))
    # Плита у цены и восемь мелких за ней — но НЕ в ближайших шести, иначе их
    # возьмёт отбор по близости, а не по размеру.
    bids = [(100.0, 10000)]
    bids += [(round(100.0 - 0.01 * i, 6), 50) for i in range(1, 12)]
    book(tr, T0, bids, asks=[(101.0, 10)])
    assert key(100.0) in tr.history.log, "плита"
    far_small = key(round(100.0 - 0.01 * 11, 6))
    assert far_small not in tr.history.log, "мелкий и далёкий журнала не заводит"
...
# ─── близость к цене: отбор журналов ──────────────────────────────────────────

def test_far_level_gets_no_log():
    """
    НАЙДЕНО НА ЖИВОЙ БИРЖЕ 03.08.

    Журналы вытеснялись 47 488 раз за 37 минут — 1 280 раз в минуту, — и история
    не накапливалась: у 15 уровней из 16 timeline был ПУСТ. Дополнение №3, самое
    важное по словам Артёма, существовало только на бумаге.

    Причина была не в потолке, а в НЕУСТОЙЧИВОСТИ критерия: «топ-5 по размеру»
    меняет состав каждый пакет, потому что размеры дышат. Близость устойчива.

    Здесь проверяется середина стакана: уровень, который НИ близко, НИ плита.
    Крупнейшие получают журнал независимо от расстояния — это отдельный тест,
    потому что карточка показывает именно плиту.
    """
    tr = tracker(history=lh.LevelLog(top_levels=20))
    assert lh.NEAR_LEVELS == 6 and lh.WALLS == 3, "тест написан под эти окна"
    # Двенадцать бидов одного размера, плита — САМАЯ ДАЛЬНЯЯ. Равные размеры
    # нужны, чтобы ENTER_SHARE никого не отсеял: тогда виден чистый эффект окон.
    bids = [(round(100.0 - 0.01 * i, 6), 1000) for i in range(12)]
    bids[11] = (bids[11][0], 90000)
    book(tr, T0, bids, asks=[(100.01, 1000)])
    logged = {k[2] for k in tr.history.log if k[1] == "bid"}
    assert 100.0 in logged, "лучший бид — в ближайших"
    assert round(100.0 - 0.01 * 5, 6) in logged, "шестой ещё в окне близости"
    assert bids[11][0] in logged, "плита — по размеру, хоть и дальше всех"
    # Середина: седьмой-одиннадцатый. Ни в окне близости, ни в трёх крупнейших.
    middle = round(100.0 - 0.01 * 8, 6)
    assert middle not in logged, "ни близко, ни крупно — журнала нет"

def test_near_level_gets_a_log_even_when_smaller():
    """Близость важнее размера: тестировать будут то, до чего цена дойдёт."""
    tr = tracker(history=lh.LevelLog(top_levels=20))
    book(tr, T0, [(100.0, 5000), (99.9, 2000)], asks=[(100.01, 1000)])
    assert key(99.9) in tr.history.log, "0.1% от цены — журнал нужен"


def test_reference_is_the_same_side_best_not_the_mid():
    """
    Первая версия мерила от СЕРЕДИНЫ рынка, и у бумаги с широким спредом обе
    стороны вылетали из окна разом: середина далеко от обеих. Цена приходит к
    уровню со своей стороны, значит и мерить надо оттуда.
    """
    tr = tracker(history=lh.LevelLog(top_levels=20))
    # Спред один процент: середина 100.5, от неё оба лучших дальше половины.
    book(tr, T0, [(100.0, 5000)], asks=[(101.0, 5000)])
    assert key(100.0) in tr.history.log, "лучший бид журнал получает"
    assert ("SBER|exchange", "ask", 101.0) in tr.history.log, "и лучший аск тоже"


def test_proximity_does_not_touch_minute_totals():
    """
    Отбор гейтит ТОЛЬКО секундный журнал. Минутный итог обязан копиться по
    всему, включая далёкие уровни, — иначе они исчезли бы и из базы, а вместе с
    ними счёт снятого и съеденного за день.
    """
    tr = tracker(history=lh.LevelLog(top_levels=20))
    book(tr, T0, [(100.0, 1000), (98.0, 9000)], asks=[(100.01, 1000)])
    far = [k for k in (tr.history.minute or {})
           if k and k[0] and k[0][1] == "bid" and k[0][2] == 98.0]
    assert far, "далёкий уровень попал в минутный итог"


def test_proximity_does_not_touch_test_counting():
    """
    САМОЕ ОПАСНОЕ ИЗ ВОЗМОЖНОГО, И Я ЭТО ПОЧТИ СДЕЛАЛ.

    Внедряя отбор, я объявил свои bb и ba перед циклом и перезаписал уже
    существующие — БЕЗ ОКРУГЛЕНИЯ. Счёт тестов сравнивает лучшую цену с ценой
    уровня из ключа, где она лежит как round(..., 6), и без округления не сошёлся
    бы НИКОГДА: ни одного теста за всё время, молча.

    Против этого стоял отдельный тест, и он сработал. Здесь закрепляется, что
    отбор журналов и счёт тестов независимы.
    """
    tr = tracker(history=lh.LevelLog(top_levels=1))   # журналов почти нет
    p = 0.1 + 0.2                                     # 0.30000000000000004
    book(tr, T0, [(0.4, 100), (p, 500)], asks=[(1.0, 100)])
    book(tr, T0 + 1, [(p, 500)], asks=[(1.0, 100)])   # цена пришла к уровню
    assert tr.levels[key(0.3)]["tests"] == 1, "тест засчитан вне зависимости от журнала"


def test_walls_are_logged_even_when_far_from_price():
    """
    НАЙДЕНО НА ЖИВОЙ БИРЖЕ, ВОПРОСОМ АРТЁМА «на биржевых данных проверил?».

    Отбор по близости я ЗАМЕНИЛ отбором по размеру вместо того, чтобы объединить.
    Механизм починился — 99 105 событий в журналах против 10 161, — а в карточке
    история осталась пустой у 14 уровней из 16:

        цена SBER 280.21 · карточка bid 280.00 (21 тик) и ask 280.74 (53 тика)
        журнал вёл ближайшие шесть: 280.20 .. 280.15

    Наборы не пересекались. Тот же дефект связности, что был между карточкой и
    сканером: два критерия «какие уровни важны», и они не совпадают.
    """
    tr = tracker(history=lh.LevelLog(top_levels=20))
    bids = [(round(100.0 - 0.01 * i, 6), 1000) for i in range(20)]
    bids[-1] = (bids[-1][0], 90000)              # плита в 19 тиках от цены
    book(tr, T0, bids, asks=[(100.01, 1000)])
    assert key(100.0) in tr.history.log, "лучший бид"
    assert key(bids[-1][0]) in tr.history.log, "плита, хоть и далеко"


def test_wall_keeps_its_log_while_being_eaten():
    """
    Плиту, которую съедают, надо довести ДО НУЛЯ: её финал и есть самое
    интересное. По пути она выпадает из первых мест по размеру, поэтому WALLS
    берётся с запасом, а не ровно один.
    """
    tr = tracker(history=lh.LevelLog(top_levels=20))
    far = round(100.0 - 0.01 * 19, 6)
    for step, size in enumerate((90000, 60000, 30000, 5000, 500)):
        bids = [(round(100.0 - 0.01 * i, 6), 1000) for i in range(20)]
        bids[-1] = (far, size)
        book(tr, T0 + step, bids, asks=[(100.01, 1000)])
    row = tr.history.log.get(key(far))
    assert row, "журнал плиты сохранился до конца"
    assert len(row) >= 3, "видно, как её ели: %d событий" % len(row)


def test_walls_cost_is_measured_and_written_down():
    src = (ROOT / "src/analysis/level_history.py").read_text()
    assert "99 105" in src, "замер с живой биржи записан рядом"
    assert "WALLS" in src
